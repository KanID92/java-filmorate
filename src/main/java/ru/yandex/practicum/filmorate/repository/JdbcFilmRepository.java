...
public class JdbcFilmRepository implements FilmRepository {
    ...
    public void deleteById(long filmId) {
        ...
        String sqlFilm_genre = "DELETE FROM film_genre WHERE film_id = :filmId";
        jdbs.update(sqlFilm_genre, Map.of("filmId", filmId));
        String sqlFilm_MPA_rating = "DELETE FROM film_MPA_rating WHERE film_id = :filmId";
        jdbs.update(sqlFilm_MPA_rating, Map.of("filmId", filmId));
...
//        String sqlTop = "SELECT * FROM FILMS AS F " +
//                "LEFT JOIN LIKES AS L ON F.FILM_ID = L.FILM_ID " +
//                "LEFT JOIN FILM_MPA_RATING AS fm ON fm.film_id = F.FILM_ID " +
//                "LEFT JOIN MPA_RATING AS mr ON mr.mpa_rating_id = fm.mpa_rating_id " +
//                "LEFT JOIN FILM_GENRE AS fg ON fg.film_id = F.FILM_ID " +
//                "LEFT JOIN GENRES AS g ON g.genre_id = fg.genre_id " +
//                "GROUP BY F.FILM_ID, L.USER_ID, FG.FILM_GENRE_ID " +
//                "ORDER BY COUNT(L.FILM_ID) DESC LIMIT :countTop";
//
//        return jdbs.query(sqlTop, Map.of("countTop", countTop), new AllFilmsExtractor()).values();
